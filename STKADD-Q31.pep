...
;
;******* Opcode 0x26
;The STKADD instruction.
;-----------------------
; Author: IffyBog
; Date modified: 4/30/23
;-----------------------
NZVC_PCB:.EQUATE 8
SP_PCB:  .EQUATE 15
RTSPadrs:.EQUATE 0
numOne:  .EQUATE 2
numTwo:  .EQUATE 4
opcode26:SUBSP   6,i         ;Set SP for the local variables
         LDWA    SP_PCB,s    ;Load the location of the RT stack pointer
         STWA    RTSPadrs,s  ;Store the Address of the RT stack pointer in the ST
         STWA    numOne,s    ;Save location for the first number's Address 
         ADDA    2,i        
         STWA    numTwo,s    ;Save location for the second number's address
         ;Now that we have the addresses of the numbers, we can extract and add them!
         LDWX    numOne,sf   ;Grab the first number 
         ADDX    numTwo,sf   ;Add the two numbers
         MOVFLGA
         STBA    NZVC_PCB,s  ;Store the flags into PCB
         ;Clean and replace the top two items with the sum 
         LDWA    0x0000,i    ;Wipe both items
         STWA    numOne,sf
         STWA    numTwo,sf
         STWX    numOne,sf   ;Store the sum at the top of the RT SP  
         ADDSP   6,i         ;popcorn
         RET
;
;******* Opcode 0x27
