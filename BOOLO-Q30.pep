...
;
;******* Opcode 0x28
;The BOOLO instruction.
;-----------------------
; Author: IffyBog
; Date modified: 4/30/23
;-----------------------
trueMsg: .ASCII  "true\x00"
falseMsg:.ASCII  "false\x00"
opcode28:LDWA    0xB,i       ;Declare i,d, and s addressing
         STWA    addrMask,d
         CALL    assertAd
         CALL    setAddr
         LDWA    opAddr,n    ;Load the operand into Acc
         ORA     0,i         ;OR to test for Zero
         BREQ    false       ;Branch for 'false' printing
         ;Print 'true'
         LDWA    trueMsg,i   ;Load the adrs of msg
         STWA    -2,s        ;Place adrs in the stack 
         SUBSP   2,i         ;popcorn
         CALL    prntMsg     ;Print the message
         ADDSP   2,i         ;Reset SP
         BR      exit
         ;Print 'false'
false:   LDWA    falseMsg,i  ;Load the adrs of msg
         STWA    -2,s        ;Place adrs in the stack 
         SUBSP   2,i         ;popcorn
         CALL    prntMsg     ;Print the message
         ADDSP   2,i         ;Reset SP
