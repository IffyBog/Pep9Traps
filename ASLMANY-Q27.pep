...
opcode28:BR      MAIN
         ...
         NZVC_PCB:.EQUATE 6
         A_PCB:  .EQUATE 7
MAIN:    LDBA    0x02,i      ;Declare ASLMANY as Direct Addr
         STWA    addrMask,d  
         CALL    assertAd
         CALL    setAddr
         SUBSP   4,i         ;Initialize variables
         LDWA    0,i   
         STWA    Vbit,s
         LDWA    A_PCB,s   
         STWA    Ashift,s
         LDWX    opAddr,n    ;Load opcode into X-reg
         ;Begin ASL loop
LOOP:    CPWX    0,i         ;Detect if the X-reg is 0 
         BREQ    EXIT        ;Exit if it is 0
         ADDX    -1,i        ;Negate 1 from X-reg to count the loop
         LDWA    Ashift,s     ;Load the Acc and shift
         ASLA
         STWA    Ashift,s    ;Store shifted val
         MOVFLGA             ;Move flags into Acc
         ORA     Vbit,s      ;Or the Vbit
         STBA    NZVC_PCB,s  ;Move the New flags into PCB
         ANDA    2,i         ;Issolate the Vbit for the next ORA
         STWA    Vbit,s
         BR      LOOP
         ;Store new values and exit out of loop
EXIT:    LDWA    Ashift,s
         STWA    A_PCB,s
         ADDSP   4,i         ;Reset SP
         RET
